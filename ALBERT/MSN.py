...
from transformers import WEIGHTS_NAME, BertConfig, BertModel, BertTokenizer, AlbertConfig,AlbertModel,AlbertTokenizer
...
class MSN(NeuralNetwork):
    ...
    def __init__(self, word_embeddings, args):
        ...
        self.albert_config = config_class.from_pretrained(args.config_name if args.config_name else args.model_name_or_path,
                                             # finetuning_task="albert_embed",
                                              cache_dir=args.cache_dir if args.cache_dir else None)
        #self.albert_config.hidden = True
        self.albert_config.num_hidden_layers=1
     #   self.albert_config.max_position_embeddings = 256
        self.albert_tokenizer = tokenizer_class.from_pretrained(
            args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
            do_lower_case=args.do_lower_case,
            cache_dir=args.cache_dir if args.cache_dir else None)

        self.albert_model = model_class.from_pretrained(args.model_name_or_path,
                                            from_tf=bool('.ckpt' in args.model_name_or_path),
                                            config=self.albert_config,
                                            cache_dir=args.cache_dir if args.cache_dir else None)
        
        self.albert_model = self.albert_model.cuda()
      ...
      #  self.word_embedding = nn.Embedding(num_embeddings=30522, embedding_dim=768, padding_idx=0)
       #                                    #,_weight=torch.FloatTensor(word_embeddings))#bert 토큰 initialization.
        ...
        self.alpha = 0.5
        self.gamma = 0.3
        self.selector_transformer = TransformerBlock(input_size=1024)
        self.W_word = nn.Parameter(data=torch.Tensor(1024, 1024, 10))
        ...
        self.transformer_utt = TransformerBlock(input_size=1024)
        self.transformer_res = TransformerBlock(input_size=1024)
        self.transformer_ur = TransformerBlock(input_size=1024)
        self.transformer_ru = TransformerBlock(input_size=1024)

        self.A1 = nn.Parameter(data=torch.Tensor(1024, 1024))
        self.A2 = nn.Parameter(data=torch.Tensor(1024, 1024))
        self.A3 = nn.Parameter(data=torch.Tensor(1024, 1024))
    ...
    def forward(self, batch):
       ...
       # for CR in batch[1]:#64개 세트.
        ...
        #with torch.no_grad():
        embedding,_=self.albert_model(**inputs)#16*256*786

        print("dd")
      #  embedding=output[-1]#output[-4]+output[-3]+output[-2]+output[-1]
        #embedding=self.word_embedding(batch[0])
        dim=list(embedding.size())
        bU_embedding= torch.zeros([dim[0],10,maxtokenlen,dim[2]],dtype=torch.float32)
        bR_embedding = torch.zeros([dim[0], maxtokenlen, dim[2]], dtype=torch.float32)
        bU_embedding = bU_embedding.cuda()
        bR_embedding = bR_embedding.cuda()
        #embedding=embedding[:,1:-1,:]
        for i, (valid_idx,utterlen_array) in enumerate(zip(batch[1],batch[3])):#batch3 단어 길이임.
            ...
            if utterlen_array[10]!=0:#response가 길이가 0?
                if (utterlen_array[10]>maxtokenlen):
                    bR_embedding[i,:,:] = embed[-utterlen_array[10]:-utterlen_array[10]+maxtokenlen, :]
                else:
                    bR_embedding[i, :utterlen_array[10], :] = embed[ -utterlen_array[10]:, :]
            # 마지막 길이 -에서 끝까지.
            lastidx=utterlen_array[10]+2#sep까지 지웠다. 중간 sep 의미.eos 도 지우니가 2 lastidx는 말그대로 뒤에서 카운팅 되는 문장의 마지막 인덱스임.
            for j in reversed(range(10)):
                start = -utterlen_array[j] - lastidx
                end = -lastidx

                if lastidx+utterlen_array[j]+1>=valid_len:
                    if valid_len-lastidx>maxtokenlen:
                        bU_embedding[i, j, :, :] = embed[0:50, :]#여기 고민.
                    else:
                        bU_embedding[i, j,-(valid_len-lastidx):, :] = embed[0:end , :]
                    break;


                if (utterlen_array[j] > maxtokenlen):
                    bU_embedding[i,j,:,:] = embed[start:start+maxtokenlen,:]
                else:
                    bU_embedding[i,j,:utterlen_array[j],:] = embed[start:end,:]
                lastidx+=utterlen_array[j]+1# eos때문에 한칸 늘려줌.
